...
#include <stdio.h>j
/*
// Patrick Ferry - 15496488 Dovydas Baranauskas - 15336731

// Add shapes using right mouse button to open menu
// Zoom in and out using F1 and F2
...
enum { SUB_SUBMENU_ENTRIES = 4 };
char *colours[SUB_SUBMENU_ENTRIES] = {"Pink", "Green", "Blue", "White"};
...
int selected_shape = 1;     //1 - cube, 2 - sphere
...
void display(void)
{
        ...
        for(int i = 0;i < totalCubes;i++){
        	...
        	if(shape_types[i] == 1)
                	...
                	DrawChair();
			//For some reason adding another menu option causes program to close
...
void processMenuEvents(int option) {
	switch (option){
		...
		case 6: selected_colour = 1;  selected_shape = 2; break;
        case 7: selected_colour = 2;  selected_shape = 2; break;
        case 8: selected_colour = 3;  selected_shape = 2; break;
        case 9: selected_colour = 4;  selected_shape = 2; break;
...
void createGLUTMenus() {
	...
	// int submenu_sizes_ball = glutCreateMenu(processMenuEvents);
    // for(int i = 0; i < SUB_SUBMENU_ENTRIES; i++)
    //     glutAddMenuEntry(colours[i], MENU_ENTRY_INDEX++);
    ...
    glutAddSubMenu("Change 2 Table", submenu_sizes_cube);
    glutAddSubMenu("Change 2 Chair", submenu_sizes_sphere);
	// glutAddSubMenu("Insert Sphere", submenu_sizes_ball);
...
{
	//Creates sphere with texture but program opens and closes so we commented it out just in case
